pipeline {
    environment {
        nexusRegapp = "44.199.200.156:8082/devops-training-app"
        nexusRegdb = "44.199.200.156:8082/devops-training-db"
        nexusRegapi = "44.199.200.156:8082/devops-training-api"
        nexusReg = "44.199.200.156:8082"
    }
    agent {
        node {
            label 'prod-stage'
...
//                    withCredentials([usernamePassword(credentialsId: 'nexusReg-Creds', passwordVariable: 'NEXUS_PASSWORD', usernameVariable: 'NEXUS_USERNAME')]) {
                    sh'docker login -u "admin" -p "123@qwer" http://44.199.200.156:8082/repository/docker-reg'
